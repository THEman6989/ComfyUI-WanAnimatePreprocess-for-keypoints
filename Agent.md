...
- **Adding new nodes:** If you add a new node, you can create it directly inside the appropriate module in `node_categories/`. You do not need to update the import statement in `nodes.py` since it uses wildcard imports (`from .node_categories.X import *`), but you **do** still need to add your new node to the `NODE_CLASS_MAPPINGS` and `NODE_DISPLAY_NAME_MAPPINGS` at the bottom of `nodes.py`.
...
- **Video helper nodes:** Video/image-batch helpers such as `FrameSubsamplerForDepth`, `FrameSubsamplerForDepthRestore`, and their batch-loop variants live in `node_categories/WanAnimatePreprocess/Video.py`. If a node pair needs to pass private workflow state, use a custom ComfyUI type name (for example `FRAME_DEPTH_SUBSAMPLE_INFO`) for the structured info object and register every public node in both mapping dictionaries in `nodes.py`. Batch loop nodes use ComfyUI dynamic graph expansion (`GraphBuilder`) rather than a literal cyclic canvas connection.
