...
class PoseAndFaceDetectionV6:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "model": ("POSEMODEL",),
                "images": ("IMAGE",),
                "width": ("INT", {"default": 832, "min": 64, "max": 2048, "step": 1, "tooltip": "Breite der Pose-Daten"}),
                "height": ("INT", {"default": 480, "min": 64, "max": 2048, "step": 1, "tooltip": "Höhe der Pose-Daten"}),
                "face_resolution": ("INT", {"default": 512, "min": 256, "max": 2048, "step": 64, "tooltip": "Auflösung des Gesichtsausschnitts (quadratisch)."}),
                "face_pad_factor": ("FLOAT", {"default": 0.3, "min": 0.0, "max": 10.0, "step": 0.05, "tooltip": "Padding um das Gesicht."}),
            },
            "optional": {
                "retarget_image": ("IMAGE", {"default": None, "tooltip": "Optionales Referenzbild"}),
            },
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess"
    DESCRIPTION = "V4 (Final): V2-Tracking Logik + Quadratischer Crop ohne Verzerrung."
    ...
    def process(self, model, images, width, height, face_resolution, face_pad_factor, retarget_image=None):
        ...
        for idx, meta in enumerate(pose_metas):
            # WICHTIG: Keypoints von normalisiert (0-1) auf Pixel (W, H) umrechnen!
            # Wir nehmen nur die Face-Keypoints (Index 23 bis Ende im 133-Format, oder was AAPoseMeta liefert)
            # AAPoseMeta liefert 'keypoints_face' als numpy array.
            
            # Koordinaten sind normalisiert, also * W und * H
            kp_face = meta['keypoints_face'][:, :2] * np.array([W, H])
            
            # --- BBox Berechnung ---
            if kp_face.shape[0] == 0:
                # Fallback, wenn keine Pose da ist
                cx, cy = W/2, H/2
                max_side = 100
                valid = False
            else:
                ...
                # Wir machen es quadratisch basierend auf der LÄNGSTEN Seite
                max_side = max(raw_w, raw_h)
                if max_side < 1: max_side = 50 # Safety
                valid = True

            # Padding anwenden (1.0 = eng, 1.3 = normal)
            square_size = max_side * (1.0 + face_pad_factor)
            half_size = square_size / 2
            
            # Exakte Koordinaten (Float für FaceInfo)
            x1 = cx - half_size
            y1 = cy - half_size
            x2 = cx + half_size
            y2 = cy + half_size
            
            # Pixel-Koordinaten für Crop (gerundet)
            ix1, iy1, ix2, iy2 = int(round(x1)), int(round(y1)), int(round(x2)), int(round(y2))
            
            # Safe Crop mit Padding (schwarze Ränder statt Verzerrung)
            # Wir holen nur das aus dem Bild, was wirklich da ist (Clamping)
            sx1 = max(0, ix1)
            sy1 = max(0, iy1)
            sx2 = min(W, ix2)
            sy2 = min(H, iy2)
            ...
            if sx2 > sx1 and sy2 > sy1:
                ...
                # Padding berechnen, um wieder auf das volle Quadrat zu kommen
                # Wenn ix1 < 0 ist (links raus), müssen wir links padden.
                pad_l = sx1 - ix1
                pad_t = sy1 - iy1
                pad_r = ix2 - sx2
                pad_b = iy2 - sy2
                
                # Ränder mit Schwarz auffüllen, damit das Bild quadratisch bleibt
                ...
                # Wenn gar nichts im Bild ist (komplett außerhalb)
                crop_img = np.zeros((face_resolution, face_resolution, C), dtype=images_np.dtype)
                valid = False

            # Auf Zielauflösung skalieren (jetzt verzerrungsfrei, da Input quadratisch ist)
            # Falls durch Rundungsfehler nicht exakt quadratisch, erzwingt Resize es jetzt (Verzerrung minimalst)
            ...
            # Info für Stitcher V3 (mit den exakten quadratischen Koordinaten)
            info_entry = {
                "frame_index": idx,
                "original_img_shape": (W, H),
                "target_tensor_size": (face_resolution, face_resolution),
                "valid": valid,
                # Wir geben dem Stitcher das volle Quadrat inkl. überstehender Bereiche
                "crop_coords": (x1, y1, x2, y2), 
                # Padding setzen wir auf 0, weil wir das Padding ins Bild "gebacken" haben (schwarze Ränder).
                # Der Stitcher projiziert einfach das Bild zurück in crop_coords.
                "padding": (0, 0, 0, 0) 
            }
        ...
        # --- 5. Output Construction (V2 Original) ---
...
class PoseAndFaceDetectionV5:
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess"
    DESCRIPTION = "V4 (Fix): Korrigiertes Tracking & Square Crop. Keine Verzerrung mehr."
    ...
    def process(self, model, images, width, height, face_resolution, face_pad_factor, retarget_image=None):
        ...
        # --- 3. Pose Extraction (V2 Original - MIT SAFETY CHECK) ---
        kp2ds = []
        for img, bbox in tqdm(zip(images_np, bboxes), total=len(images_np), desc="V4 Keypoints"):
            # V2 Safety Check: Verhindert Absturz bei 0-Größe BBoxen
            if bbox is None or bbox[-1] <= 0 or (bbox[2] - bbox[0]) < 10 or (bbox[3] - bbox[1]) < 10:
                bbox = np.array([0, 0, img.shape[1], img.shape[0]])
        ...
        # --- 4. Face Extraction (KORRIGIERT: Pixel-Koordinaten & Square Crop) ---
        face_images = []
        face_bboxes = []
        face_info = []
        ...
        for idx, meta in enumerate(pose_metas):
            ...
            # Safe Crop mit Padding (schwarze Ränder statt Verzerrung)
            # Wir holen nur das aus dem Bild, was wirklich da ist
            ...
            if sx2 > sx1 and sy2 > sy1:
                ...
                # Padding berechnen, um auf das perfekte Quadrat zu kommen
                pad_l = sx1 - ix1
                pad_t = sy1 - iy1
                pad_r = ix2 - sx2
                pad_b = iy2 - sy2
                
                # Ränder mit Schwarz auffüllen
                if any([pad_l > 0, pad_t > 0, pad_r > 0, pad_b > 0]):
                    crop_img = cv2.copyMakeBorder(
                        crop_img, 
                        max(0, pad_t), max(0, pad_b), max(0, pad_l), max(0, pad_r), 
            ...
            # Auf Zielauflösung skalieren (jetzt verzerrungsfrei, da Input quadratisch)
            face_image_resized = cv2.resize(crop_img, (face_resolution, face_resolution), interpolation=cv2.INTER_CUBIC)
            face_images.append(face_image_resized)
            face_bboxes.append((ix1, iy1, ix2, iy2))
...
class PoseAndFaceDetectionV4:
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess"
    DESCRIPTION = "V4: Exakte V2 Logik (Frame-by-Frame), aber mit einstellbarer Auflösung und kompatibel mit Stitcher V3."
    ...
    def process(self, model, images, width, height, face_resolution, face_pad_factor, retarget_image=None):
        ...
        for img in tqdm(images_np, total=len(images_np), desc="V4 Detecting"):
            ...
            # Nimm immer die erste BBox (V2 Verhalten)
            bbox_res = det_result[0][0]["bbox"]
            bboxes.append(bbox_res)
        ...
        # --- 4. Face Extraction (V2 Logic + Resolution & Info) ---
        face_images = []
        face_bboxes = []
        face_info = [] # Das ist neu für den Stitcher V3

        for idx, meta in enumerate(pose_metas):
            # V2 nutzt standardmäßig scale=1.3. 
            # Wir erlauben Anpassung: 1.0 + pad_factor (0.3 -> 1.3)
            current_scale = 1.0 + face_pad_factor
            
            # Original V2 Funktion für BBox
            face_bbox_for_image = get_face_bboxes(meta['keypoints_face'][:, :2], scale=current_scale, image_shape=(H, W))
            ...
            valid = True
            # V2 Fallback Logic (falls kein Gesicht gefunden/leer)
            if face_image.size == 0 or face_image.shape[0] == 0 or face_image.shape[1] == 0:
                ...
                if face_image.size == 0:
                    face_image = np.zeros((face_resolution, face_resolution, C), dtype=images_np.dtype)
            
            # Resize auf die gewünschte Resolution (statt fix 512)
            face_image_resized = cv2.resize(face_image, (face_resolution, face_resolution))
            face_images.append(face_image_resized)
            face_bboxes.append((x1, y1, x2, y2))
            
            # --- Erstelle Info für Stitcher V3 ---
            # V2 Logic clippt Koordinaten, benutzt kein Padding (schwarze Ränder).
            # Daher padding=(0,0,0,0) und die exakten crop-Koordinaten.
            info_entry = {
                "frame_index": idx,
                "original_img_shape": (W, H),
                "target_tensor_size": (face_resolution, face_resolution),
...
class PoseDataAdaptiveUpperBodyOffsetHelper:
            ...
            "required": {
                ...
                "start_seconds": (
                    "FLOAT",
                    {
                        ...
                        "max": 3600.0,
                        "step": 0.01,
                        "tooltip": "Startzeitpunkt für die Analyse der Fußposition.",
                    },
                ),
                "duration_seconds": (
                    "FLOAT",
                    {
                        ...
                        "max": 60.0,
                        "step": 0.01,
                        "tooltip": "Dauer der Analyse. Dieser Wert wird auch als output durchgereicht.",
                    },
                ),
                "person_height_m": (
                    "FLOAT",
                    {
                        ...
                        "max": 3.0,
                        "step": 0.01,
                        "tooltip": "Die tatsächliche Größe der Person in Metern (z.B. 1.70).",
                    },
                ),
                "canvas_height_m": (
                    "FLOAT",
                    {
                        ...
                        "max": 10.0,
                        "step": 0.01,
                        "tooltip": "Die vertikale Höhe des sichtbaren Bildausschnitts in Metern an der Position der Person. Bleibt dieser Wert gleich, bleibt die Person gleich groß, auch wenn sie im Bild wandert.",
                    },
                ),
                "fps": (
                    "INT",
                    {
                        ...
                        "max": 960,
                        "step": 1,
                        "tooltip": "Framerate zur Berechnung der Frames.",
                    },
                ),
            }
        }
    ...
    RETURN_NAMES = ("head_padding", "duration_seconds", "info")
    FUNCTION = "calculate"
    CATEGORY = "WanAnimatePreprocess"
    DESCRIPTION = (
        "Berechnet das Head-Padding relativ zur AKTUELLEN Fußposition. "
        "Die Füße bleiben dort, wo sie sind. Der Kopf wird so platziert, dass die Proportionen stimmen."
    )
    ...
    def calculate(
        ...
        if not pose_metas:
            return (0.0, duration_seconds, "No pose data found.")
        ...
        # 1. Analyse der Fußposition
        # Wir suchen den tiefsten Punkt der Füße (Y-Koordinate) im Analyse-Zeitraum.
        foot_y_values = []
        
        # Indizes für Füße (Ankles, Toes, Heels)
        foot_indices = [10, 13, 18, 19, 20, 21, 22, 23, 24]

        for i in range(start_frame, end_frame):
            meta = pose_metas[i]
            if not hasattr(meta, "kps_body") or meta.kps_body is None:
                continue
            ...
            for idx in foot_indices:
                if idx < len(meta.kps_body):
                    pt = meta.kps_body[idx]
                    if pt is not None:
                        # Handle numpy array or list/tuple
                        y = pt[1] if isinstance(pt, (list, tuple, np.ndarray)) else None
                        score = pt[2] if isinstance(pt, (list, tuple, np.ndarray)) and len(pt) > 2 else 1.0
                        
                        if y is not None and score > 0.05:
                            # Normalisieren
                            y_norm = y / height
                            if y_norm > max_y:
                                max_y = y_norm
            ...
            if found:
                foot_y_values.append(max_y)

        # Wenn keine Füße erkannt wurden, nehmen wir einen Fallback (z.B. 0.95)
        avg_foot_y = np.median(foot_y_values) if foot_y_values else 0.95
        
        # 2. Berechnung der Ziel-Größe im Bild (Ratio)
        # Beispiel: 1.70m Person in 2.20m hohem Bild = 0.77 (77% der Bildhöhe)
        visual_height_ratio = person_height_m / canvas_height_m
        
        # 3. Berechnung des Head Paddings
        # Wo muss der Kopf sein, wenn die Füße bei 'avg_foot_y' bleiben?
        # Head Y = Fuß Y - Körpergröße
        target_head_y = avg_foot_y - visual_height_ratio
        
        # Padding ist der Abstand von oben (0.0) zum Kopf
        # Wir clippen nicht zu hart, erlauben aber keine negativen Werte (außer du willst Overflow erlauben)
        # Hier clippen wir auf 0.0, damit der Kopf nicht aus dem Bild fliegt, 
        # es sei denn, die 'Head Padding Allow Overflow' Option in der Editor Node ist an.
        # Da diese Node nur den Wert liefert, geben wir den echten Wert aus, aber >= 0 ist meist sicherer.
        calculated_head_padding = max(0.0, target_head_y)

        info = (
            f"Analysis Frame: {start_frame}-{end_frame}\n"
            f"Avg Foot Y (Anchor): {avg_foot_y:.4f}\n"
            f"Visual Ratio: {visual_height_ratio:.4f} ({person_height_m}m / {canvas_height_m}m)\n"
            f"Result Head Padding: {calculated_head_padding:.4f}"
        )

        return (calculated_head_padding, duration_seconds, info)

class PoseDataAutomaticOffsetNode:
    """
    Positioniert eine Person basierend auf ihrer Körpergröße in einem virtuellen Canvas (Kameraausschnitt).
    Analysiert kurz die Position, berechnet den Offset und 'lockt' diesen dann, um Wackeln zu verhindern.
            ...
            "required": {
                ...
                "foot_padding": (
                    "FLOAT",
                    {
                        ...
                        "max": 0.5,
                        "step": 0.001,
                        "tooltip": "Abstand der Füße vom unteren Bildrand (normalisiert 0-1).",
                    },
                ),
                "fps": (
                    "INT",
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess"
    DESCRIPTION = "Smart offset for scaling specific body heights into a defined canvas height with stabilization."

    def process(self, pose_data, source_height, canvas_height, analysis_duration, foot_padding, fps):
        pose_data_copy = copy.deepcopy(pose_data)
        pose_metas = pose_data_copy.get("pose_metas", [])
...
class PoseAndFaceDetectionV2:
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess"
    DESCRIPTION = "Detects human poses and face images. V2: Returns a list of BBOXes for ALL frames."
    ...
    def process(self, model, images, width, height, retarget_image=None):
        ...
        for img in tqdm(images_np, total=len(images_np), desc="Detecting bboxes"):
            ...
            # YOLO liefert eine Liste von Listen. Wir nehmen hier bbox[0][0] (erste Person im Batch, erste Detektion)
            # Falls nichts erkannt wurde, kommt oft eine Default-Box zurück, das prüft der Detektor meist intern.
            bbox_res = det_result[0][0]["bbox"]
            bboxes.append(bbox_res)
        ...
        for idx, meta in enumerate(pose_metas):
            ...
            if face_image.size == 0 or face_image.shape[0] == 0 or face_image.shape[1] == 0:
                # Fallback wenn Gesichtscrop leer
                fallback_size = int(min(H, W) * 0.3)
                fallback_x1 = (W - fallback_size) // 2
                fallback_x2 = fallback_x1 + fallback_size
                fallback_y1 = int(H * 0.1)
        ...
        # --- ÄNDERUNG FÜR V2: Alle BBoxen sammeln ---
        final_bboxes_list = []
        for bb in bboxes:
            # Sicherstellen, dass es ein flaches Array ist
            bb_flat = np.array(bb).flatten()
            # Mindestens 4 Werte (x, y, w, h, ...)
            if bb_flat.shape[0] >= 4:
                # Konvertierung zu int-Tupel (x, y, w, h)
                bbox_ints = tuple(int(v) for v in bb_flat[:4])
            else:
                bbox_ints = (0, 0, 0, 0)
            final_bboxes_list.append(bbox_ints)
        ...
        # Keypoints Logik für JSON output (unverändert)
        key_frame_num = 4 if B >= 4 else 1
        key_frame_step = len(pose_metas) // key_frame_num
        key_frame_index_list = list(range(0, len(pose_metas), key_frame_step))
        key_points_index = [0, 1, 2, 5, 8, 11, 10, 13]
        ...
        points_dict_list = [] 
        # Hinweis: Im Original-Code wird points_dict_list in der Loop überschrieben und nur der letzte Stand genommen,
        # oder es war unvollständig im Snippet. Ich behalte die Logik bei, dass es am Ende json dumped.
        # Da im Original-Code `points_dict_list` referenziert wird, initialisieren wir es hier sauber.
        
        # Wir nehmen einfach die Points vom ersten Keyframe für den Output, 
        # oder sammeln alle (im Original war die Logik etwas spärlich).
        # Hier der Original-Block rekonstruiert:
        for key_frame_index in key_frame_index_list:
            keypoints_body_list = []
            if key_frame_index < len(pose_metas):
                body_key_points = pose_metas[key_frame_index]['keypoints_body']
                ...
                wh = np.array([[pose_metas[0]['width'], pose_metas[0]['height']]])
                points = (keypoints_body * wh).astype(np.int32)
                # Reset list for each keyframe to match original logic behavior (last one wins usually unless accumulated)
                # Aber für JSON dump nehmen wir an, wir wollen eine Liste von Punkten.
                # Der Originalcode hat `points_dict_list` in der Loop neu befüllt.
                points_dict_list = [] 
                for point in points:
                    points_dict_list.append({"x": int(point[0]), "y": int(point[1])})
        ...
        return (pose_data, face_images_tensor, json.dumps(points_dict_list), final_bboxes_list, face_bboxes)
